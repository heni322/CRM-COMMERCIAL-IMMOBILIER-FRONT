...
import { useCreateProperty, useGetPropertyById, useUpdateProperty } from 'src/services/properties.service'
...
const UpdateForm = ({
  propertyDataIsFetching = false,
  propertyDataIsSuccess = false,
  propertyData,
  propertyId,
  blocsData,
  residence,
  ...
  useEffect(() => {
    if (propertyDataIsSuccess) {
      setFormInput({
        ...propertyData
      })
    }
  }, [propertyDataIsFetching])

  const handleSubmit = async () => {
    setLoading(true)

    try {
      const response = await updatePropertyMutation?.mutateAsync({ id: propertyData?.id, values: formInput })
      setLoading(false)
      if (!residence?.id) {
  ...
  const handleDeletePiece = async piece => {
    try {
      ...
      await deletePieceMutation.mutateAsync({ entitled: piece.entitled })
    } catch (error) {
          ...
          {!residence?.id && (
            <FormControl fullWidth>
              <Autocomplete
                ...
                value={getResidences?.data?.find(res => res.id === formInput.d_project_id) || null}
        ...
        <Grid item xs={12} sm={4}>
          <FormControl fullWidth>
            <Autocomplete
              ...
              value={getBlocs?.data?.find(bloc => bloc.id === formInput.d_bloc_id) || null}
        ...
        <Grid item xs={12} sm={4}>
          <FormControl fullWidth>
            <Autocomplete
              ...
              value={(Array.isArray(typesData) ? typesData : (typesData?.data ?? [])).find(type => type.id === formInput.p_property_type_id) || null}
        ...
        <Grid item xs={12} sm={4}>
          <FormControl fullWidth>
            <Autocomplete
              ...
              value={(Array.isArray(usagesData) ? usagesData : (usagesData?.data ?? [])).find(usage => usage.id === formInput.p_usage_type_id) || null}
