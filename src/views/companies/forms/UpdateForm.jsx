...
import { useCreateCompany, useGetCompaniesById, useUpdateCompany } from 'src/services/companies.service'
...
const UpdateForm = Company => {
  ...
  const getCompanyQuery = useGetCompaniesById(Company.companyId)
  const companyData = getCompanyQuery?.data
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      ...
      formData.append(`logo`, files?.length === 0 ? '' : files[0])
      ...
      try {
        await updateCompany.mutateAsync({ data: formData, id: Company.companyId })
  ...
  useEffect(() => {
    if (getCompanyQuery.isSuccess) {
      formik.setValues(prevValues => {
        return { ...prevValues, ...companyData, logo: null }
      })
      setIsLoading(false)
    }
  }, [companyData, getCompanyQuery.isSuccess])
          ...
          <Form autoComplete='off' noValidate onSubmit={handleSubmit}>
            <Grid container spacing={5}>
              ...
              <Grid item xs={12} sm={12}>
                ...
                <img src={formik?.values?.logo_url} alt='logo' />
                <CustomFileUpload
